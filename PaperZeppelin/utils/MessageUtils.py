...
async def gen_group_help(help_command, group: commands.Group) -> str:
    ...
    message += f"You can get more info about a command (params and subcommands) by using '{help_command.context.clean_prefix}help <command>'\nCommands followed by ↪ have subcommands."
    message += "\n```"
    return message if len(filtered) > 0 or group.cog.qualified_name == "Basic" else "🔒 You do not have permission to view this command"
...
def build(**kwargs):
    ...
    if type is None:
        ...
        if type == "user_info":
            ...
            return presence_update(before=kwargs.get("before"), after=kwargs.get("after"))

def user_info(member: typing.Union[discord.User, discord.Member], issuer: discord.User) -> discord.Embed:
    now = datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc)
    is_member = isinstance(member, discord.Member)

    embed = discord.Embed(colour=member.top_role.colour if is_member else 0x00cea2, timestamp=now)

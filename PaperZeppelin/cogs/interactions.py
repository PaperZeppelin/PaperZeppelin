    ...
    async def on_interaction(self, interaction: Interaction):
        if interaction.data["name"] == "presence":
            if interaction.data["options"][0]["name"] == "status":
                ...
                await interaction.response.send_message(embed=MessageUtils.build(type="status_update", before=client_status.value, after=status))
                ...
                await self.client.change_presence(activity=activity, status=self.client.status)
                await interaction.response.send_message(embed=MessageUtils.build(type="presence_update", before=client_activity, after=activity))
            ...
            if "members" in interaction.data["resolved"] and target in interaction.data["resolved"]["members"]:
                ...
                member = discord.Member(data=member_dict, guild=interaction.guild, state=interaction._state)
            await interaction.response.send_message(embed=MessageUtils.build(type="user_info", member=member, issuer=interaction.user), ephemeral=True)

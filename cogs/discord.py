...
class Discord(commands.Cog):
    ...
    def datetime_from_snowflake(self, snowflake: int) -> datetime.datetime:
        return datetime.datetime.fromtimestamp(
            floor(int((snowflake >> 22) + DISCORD_EPOCH) / 1000)
        ).replace(tzinfo=None)
    ...
    async def snowflake(self, ctx: commands.Context, snowflake: int):
        ...
        if ctx.invoked_subcommand is None:
            await ctx.send(
                ...
                .set_image(
                    url="https://discord.com/assets/94722171abc49573d1a129e2264da4ad.png"
                )
                .add_field(
                    name="Timestamp",
                    value=f"{self.timestamp_from_snowflake(snowflake)} (<t:{self.timestamp_from_snowflake(snowflake)}:F>)",
                )
            )
